...
const App = {
  async refreshBookings(){
    try{
      ...
      // ✅ توحيد عربي/إنجليزي
      State.bookings = (Array.isArray(bookings) ? bookings : []).map(b=>({
        ...b,
        bookingDate: b["تاريخ الحجز"] ?? b.bookingDate,
        period: b["الحصة"] ?? b.period,
        name: b["الاسم"] ?? b.name,
        subject: b["المادة"] ?? b.subject,
        grade: b["الصف"] ?? b.grade,
        lessonTitle: b["عنوان الدرس"] ?? b.lessonTitle,
      }));
  ...
  // ✅ Toast + تعارض + تفريغ الحقول
  async submitBooking(){
    try{
      const payload = {
        "تاريخ الإنشاء": new Date().toISOString(),
        "الاسم": $("b_name").value.trim(),
        "المادة": $("b_subject").value.trim(),
        "الصف": $("b_grade").value.trim(),
        "عنوان الدرس": $("b_lessonTitle").value.trim(),
        "الهدف من الحجز": $("b_purpose").value.trim(),
        "تاريخ الحجز": $("b_date").value,
        "الحصة": Number($("b_period").value),
        "ملاحظات": $("b_notes").value.trim()
      };

      const required = ["الاسم","المادة","الصف","عنوان الدرس","الهدف من الحجز","تاريخ الحجز","الحصة"];
      for(const k of required){
        if(!payload[k]) return toast("الرجاء تعبئة جميع الحقول", "error");
      }
